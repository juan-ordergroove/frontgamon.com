...
function openModal(href, height, width)
{
				var div = "<div id='modalDialog' style='height: 100%; text-align: center;'><img src='/images/ajax-loader.gif' alt='' id='ajax-loader'></div>";
				
				var Dom = YAHOO.util.Dom;
				var top;
				var left;
				
				top = Dom.getViewportHeight() - height;
				top /= 2;
				top = (top/Dom.getViewportHeight()) * 100;
				
				left = 300 + Dom.getViewportWidth()/2 - width/2;
				
				$(div).modal(
				{
								containerCss: {'background-color': '#d3d3d3',
																'height': (height + 5) + 'px',
																'width': (width + 5) + 'px',
																'left': left + 'px',
																'top': top + '%',
				...
				var iFrame = "<div id='modalDivContent' width=" + width + " height=" + height +" style='display: none'>";
				var result = $.get(href, function(data){
												iFrame += data.replace(/\n/g, "");
												iFrame += "</div>";
												$("#modalDialog").append(iFrame);
												setTimeout("$('#modalDivContent').css('display', '');", 1500);
												setTimeout("$('#ajax-loader').remove()", 1500);
								});
}
