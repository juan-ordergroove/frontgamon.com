...
class dump {
	function dump ($var, $name = '') {
        ...
        echo "<pre style='$style'>" .
					($name != '' ? "$name : " : '') .
					dump::_get_info_var ($var, $name) .
					"</pre>";
	}
    
	function get ($var, $name = '') {
		return ($name != '' ? "$name : " : '') . dump::_get_info_var ($var, $name);
	}
		...
		if (is_array ($var)) {
			...
			foreach (array_keys ($var) as $key) {
				...
				if (($indent == 0) && ($name != '') && (! is_int ($key)) && ($name == $key)) {
					...
					$out .= dump::_get_info_var ($var[$key], '', $indent + 1);
				}
			}
			$out .= "$spc)";
			...
			while (list($prop, $val) = each($arr)) {
				...
				$out .= dump::_get_info_var ($val, $name != '' ? $prop : '', $indent + 1);
			}
