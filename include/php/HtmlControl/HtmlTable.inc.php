...
class HtmlTable extends HtmlControl
{
    ...
    */
    function HtmlTable ($id = '')
    {
        ...
        $this->fstyles = array();
								...
								$this->table[0]["table_values"]["id"] = $id;
    }
    ...
    */
    ...
    function SetRowContent ( $row, $content )
    {
		...
		function SetRowAttributes ($row, $attributes)
		{
			if (count($attributes))
			{
				if (!isset($this->table[$row]['attributes']))
					$this->table[$row]['attributes'] = array();
				$this->table[$row]['attributes'] = array_merge($this->table[$row]['attributes'], $attributes);
			}
		}
    ...
    */
    function SetRowStyle ( $row, $attributes )
    {
        if ( is_array( $row ) )
        {
            foreach ( $row as $num )
    ...
    * @param    array                $attributes
    */
    function SetColStyle ( $col, $attributes )
    {
        if ( is_array( $col ) )
        {
            foreach ( $col as $num )
        ...
        for ( $row = 1; $row <= $this->row_count; $row++ )
								{
										...
										$content .= "\t<tr";
										if (isset($this->table[$row]["attributes"]))
												foreach ($this->table[$row]["attributes"] as $key => $value)
														$content .= " $key='$value'";
										$content .= ">\n";
										...
										for ( $col = 1; $col <= $count; $col++ )
          {
												...
												$td_array = (isset($this->default_settings["td"]) ? $this->default_settings["td"] : '');
												...
												if (strlen($t_string))
												{
														$t_string = ($t_string[strlen($t_string)-1] != "\n" ? $t_string . "\n" : $t_string);
														$content .= ">\n\t\t\t" . str_replace(array("\n", "\r", "\n\r"), "\n\t\t", $t_string) . "</td>\n";
												}
												else
														$content .= "></td>\n";
